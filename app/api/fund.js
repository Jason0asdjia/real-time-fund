...
const requestFundGzData = (code) => new Promise((resolve, reject) => {
  ...
  iframe.onload = () => {
    try {
      ...
      if (!frameWindow || !frameDocument) {
        finish(reject, new Error('基金估值容器初始化失败'));
        return;
      }
      ...
      frameWindow.jsonpgz = (json) => {
        if (!json || typeof json !== 'object') {
          finish(reject, new Error('基金估值数据无效'));
          return;
        }
      ...
      script.async = true;
      script.onerror = () => finish(reject, new Error('基金数据加载失败'));
  ...
  timer = window.setTimeout(() => {
    finish(reject, new Error('基金估值请求超时'));
  }, FUND_GZ_TIMEOUT_MS);
});
...
export const fetchFundData = async (c, options = {}) => {
  ...
  const holdingsPromise = !includeHoldings
    ? Promise.resolve({
  ...
  if (!gzData) {
    return fetchFundDataFallback(c);
  }
